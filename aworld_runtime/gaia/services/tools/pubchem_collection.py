...
class PubChemCollection(ActionCollection):
    ...
    def __init__(self, arguments: ActionArguments) -> None:
        ...
        self.session = httpx.AsyncClient(
            headers={
                "User-Agent": "AWorld-PubChem-MCP/1.0 (https://github.com/aworld-framework)",
    ...
    async def _rate_limit(self) -> float:
    ...
    async def _make_request(self, url: str, params: dict = None) -> tuple[dict | None, float]:
        ...
        Raises:
            httpx.RequestError: For API request failures
        """
        ...
        try:
            ...
            response_time = time.time() - start_time
            raise httpx.RequestError(f"Request timeout after {self.timeout}s") from e
    ...
    async def mcp_search_compounds(
        ...
        try:
            ...
            # Make API request
            data, response_time = await self._make_request(url)
    ...
    async def mcp_get_compound_properties(
        self,
        cid: int = Field(description="PubChem Compound ID (CID)"),
    ...
    async def mcp_search_similar_compounds(
        ...
        try:
            ...
            # Make API request
            data, response_time = await self._make_request(url, params)
    ...
    async def mcp_get_pubchem_capabilities(self) -> ActionResponse:
