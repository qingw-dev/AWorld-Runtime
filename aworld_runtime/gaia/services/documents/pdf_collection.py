...
try:
    ...
    subprocess.run(["uv", "pip", "install", "marker-pdf==1.8.1"])
    print("Marker package not found. Try to install it to use PDFDocumentCollection.")


class PDFDocumentCollection(ActionCollection):
    ...
    async def _load_marker_models(self) -> None:
        ...
        if not self._models_loaded:
            try:
                ...
                self._marker_models = await asyncio.to_thread(create_model_dict)
    ...
    async def _extract_content_with_marker(
        ...
        for idx, (page_num, image_data) in enumerate(images.items()):
            try:
                ...
                if hasattr(image_data, "save"):
                    # PIL Image object
                    await asyncio.to_thread(image_data.save, image_path)
    ...
    async def _format_content_for_llm(self, content: str, output_format: str) -> str:
...
if __name__ == "__main__":
    ...
    try:
        service = PDFDocumentCollection(args)
        service.run()
    except Exception as e:
