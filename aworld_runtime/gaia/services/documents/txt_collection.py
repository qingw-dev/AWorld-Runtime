...
class TextCollection(ActionCollection):
    ...
    async def _detect_encoding(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            async def read_raw_data() -> bytes:
                # Read file in binary mode for encoding detection
                with open(file_path, "rb") as f:
                    return f.read()

            raw_data = await asyncio.to_thread(read_raw_data)
    ...
    async def mcp_extract_text_content(
        ...
        try:
            ...
            # Validate input file
            file_path: Path = await self._validate_file_path(file_path)
            self._color_log(f"Processing text document: {file_path}", Color.blue)
    ...
    async def mcp_list_supported_formats(self) -> ActionResponse:
        ...
        Returns:
            ActionResponse with a list of supported file extensions.
        """
        return ActionResponse(
            content={
