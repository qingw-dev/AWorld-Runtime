...
class ImageCollection(ActionCollection):
    ...
    async def _load_image(self, file_path: Path) -> Image.Image:
        ...
        """
        try:
            return await asyncio.to_thread(Image.open, file_path)
    ...
    async def _analyze_with_ai(self, image_base64: str, task: str) -> str:
    ...
    async def mcp_analyze_image_ai(
        self,
        ...
        try:
            ...
            # Perform AI analysis
            analysis_result = await self._analyze_with_ai(image_base64, task)
            processing_time = time.time() - start_time
    ...
    async def mcp_get_image_metadata(
        self,
