...
class AudioCollection(ActionCollection):
    ...
    async def _prepare_audio_for_transcription(self, file_path: Path) -> Path:
    ...
    async def _transcribe_with_whisper(self, audio_path: Path) -> dict[str, Any]:
    ...
    async def _get_audio_metadata(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                self.logger.warning(f"Failed to extract metadata: {stderr.decode()}")
                return {}
    ...
    async def _trim_audio(self, input_path: Path, start_time: float, duration: float | None = None) -> Path:
        ...
        if process.returncode != 0:
            raise RuntimeError(f"Audio trimming failed: {stderr.decode()}")
    ...
    async def mcp_transcribe_audio(
        ...
        try:
            ...
            original_metadata = await self._get_audio_metadata(file_path)

            # Prepare audio for transcription
            prepared_audio = await self._prepare_audio_for_transcription(file_path)

            # Perform transcription
            transcription_result = await self._transcribe_with_whisper(prepared_audio)
    ...
    async def mcp_extract_audio_metadata(
        self,
    ...
    async def mcp_trim_audio(
        ...
        try:
            ...
            # Trim audio
            output_path = await self._trim_audio(file_path, start_time, duration)

            # Get trimmed file metadata
            trimmed_metadata = await self._get_audio_metadata(output_path)
